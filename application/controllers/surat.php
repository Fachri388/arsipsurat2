...
class Surat extends CI_Controller {
	...
	public function index(){
		if($this->session->userdata('logged_in') ==  TRUE){
			if($this->session->userdata('jabatan') == 'Admin'){
				...
				$data['main_view'] = 'pegawai/surat_masuk';
	...
	public function surat_masuk_user(){
		if($this->session->userdata('logged_in') == TRUE){
			...
			if($this->session->userdata('jabatan') == 'Admin'){
				...
				if ($this->form_validation->run() == TRUE) {
					...
					if ($this->upload->do_upload('file_surat')){
						...
						if($this->surat_model->tambah_surat_masuk($this->upload->data()) == TRUE ){
							$this->session->set_flashdata('notif', 'Tambah surat berhasil!');
							redirect('surat/surat_masuk_user');			
						} else {
							$this->session->set_flashdata('notif', 'Tambah surat gagal!');
							redirect('surat/surat_masuk_user');			
						}
	...
	public function ubah_surat_masuk_user(){
		if($this->session->userdata('logged_in') == TRUE){

			if($this->session->userdata('jabatan') == 'User'){

				$this->form_validation->set_rules('edit_no_surat_user', 'No.Surat', 'trim|required');
				$this->form_validation->set_rules('edit_no_surat_user', 'Tgl.Kirim', 'trim|required|date');
				$this->form_validation->set_rules('edit_no_surat_user', 'Tgl.Kirim', 'trim|required|date');
				$this->form_validation->set_rules('edit_no_surat_user', 'Pengirim', 'trim|required');
				$this->form_validation->set_rules('edit_no_surat_user', 'Penerima', 'trim|required');
				$this->form_validation->set_rules('edit_no_surat_user', 'Perihal', 'trim|required');

				if($this->form_validation->run() == TRUE){
					if($this->surat_model->ubah_surat_masuk_user() == TRUE){
						$this->session->set_flashdata('notif','ubah berhasil');	
						redirect('surat/surat_masuk_user','refresh');
					} else{
						$this->session->set_flashdata('notif', 'ubah gagal');
						redirect('surat/surat_masuk_user','refresh');
					}
	...
	public function ubah_file_surat_masuk_user(){
		if($this->session->userdata('logged_in') == TRUE){
			if($this->session->userdata('jabatan') == 'User'){
				$config['upload_path'] = './uploads/';
				$config['allowed_types'] = 'pdf';
				...
				$this->load->library('upload', $config);
				if($this->upload->do_upload('edit_file_surat_user')){
					if($this->surat_model->ubah_file_surat_masuk_user($this->upload->data()) == TRUE){
						$this->session->set_flashdata('notif', 'ubah file gagal');
						redirect('surat/surat_masuk_user','refresh');
					} else{
						$this->session->set_flashdata('notif', 'ubah file berhasil');
	...
	public function search(){
		...
		$data['main_view'] = 'pegawai/surat_masuk2';
		$data['data_surat_masuk'] = $this->surat_model->get_keyword($keyword);
	...
	public function searchSuratKeluar(){
		...
		$data['main_view'] = 'admin/surat_keluar_view2';
		$data['data_surat_keluar'] = $this->surat_model->get_search_surat_keluar($keyword);
